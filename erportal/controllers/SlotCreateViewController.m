...
@interface SlotCreateViewController (){
...
@implementation SlotCreateViewController
...
- (void)viewDidLoad {
    ...
    self.queueName.text = self.queue.name;
    self.queueSpeciality.text = self.queue.speciality;
    self.dateSlot.text = self.slot.date;
    self.timeSlot.text = self.slot.time;
    [self setFieldValue];
...
- (IBAction)finishTalon:(UIButton *)sender {
    [self setProfileValue];
    if ([self.profileEntity validate]) {
        service.profile = self.profileEntity;
        service.queue = self.queue;
        service.slot = self.slot;
        service.controller = self;
        [service createTalon];
    }else{
        [self presentViewController:[PSIAlertHelper viewAlertBasicMessage:self.profileEntity.error] animated:true completion:nil];
    }
}
...
- (void)prepareForSegue:(UIStoryboardSegue *)segue sender:(id)sender {
    ...
    [nextController setTalon:self.talon];
